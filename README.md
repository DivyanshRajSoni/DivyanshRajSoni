 # 👋😄 Hi, I’m **Divyansh Raj Soni** 🚀💻

[![GitHub followers](https://img.shields.io/github/followers/DivyanshRajSoni?label=Followers&style=social)](https://github.com/DivyanshRajSoni)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-blue)](https://www.linkedin.com/in/divyanshrajsoni)

---

## 🧠✨ About Me

I’m a **builder-first developer** 🛠️ who loves turning ideas 💡 into **real, working systems**.  
From **AI-powered applications 🤖** to **backend APIs ⚙️** and **interactive UIs 🎨**, I focus on hands-on learning and real-world impact 🚀.

- 🔥 Passionate about **AI, Backend & System Design**
- 🧩 Love solving real-world problems
- 🌱 Learning deeply, not superficially
- 🤝 Open to collaboration & open-source 🌍

---

## 🛠️💻 Skills (Based on Real Projects)

### 🔤 Programming Languages
- ✨ C++
- 🟨 JavaScript
- 🐍 Python
- 🌐 HTML, CSS

### ⚙️ Backend & APIs
- 🟢 Node.js
- 🚂 Express.js
- 🔌 REST API Development
- ☁️ Serverless Functions (Vercel)

### 🤖🧠 AI / Machine Learning
- 🎨 Stable Diffusion
- 🔁 Transformers
- 👁️ Vision Transformers (ViT)
- 💬 GPT-based Models
- 🖼️ Image Generation & Captioning
- 🔥 PyTorch
- ⚡ CUDA
- 🤗 Hugging Face Models

### 🎨 Frontend
- ✨ Vanilla JavaScript
- 💎 Tailwind CSS
- 🖥️ Interactive UI/UX
- 📊 Data Visualization

### 🧰 Tools & Technologies
- 🌶️ Flask
- 🐙 Git & GitHub
- 📄 NDJSON
- 🔗 API Integrations (USDA API, Gemini API)
- 🔐 Security (CORS, Helmet, Morgan)

---

## 📂🚀 Featured Projects

### 🥗🤖 ProductLens – AI-Powered Product Search & Health Analysis
- ⚙️ Node.js + Express REST APIs
- 🧠 USDA Food API Integration
- ☁️ Vercel Serverless Functions
- ❤️ Health & product intelligence

### 🔤🎨 VISUAL-ALPHABET
- 🤖 AI-generated visual alphabets
- 🎨 Stable Diffusion + Transformers
- 👁️ ViT + GPT Models
- 🔥 PyTorch + CUDA
- 🌶️ Flask ML backend

### 🌍♻️ CarbonFoot
- 🌱 Carbon footprint calculator
- ⚙️ Node.js + Express backend
- 🔐 Secure middleware setup
- 🌎 Sustainability focused

### 🔐🔑 Strong Password Generator (Context-Aware)
- 🧠 Context-based password logic
- 🔒 Security-focused design
- ⚙️ Smart generation rules

### 🚀🌌 Kardashev Scale Simulator
- 📊 Interactive science simulation
- 🖥️ HTML, CSS, JavaScript
- 🎨 Clean UI + visualization

### 🧠⚔️ Tactical Command Center
- 🪖 Strategy & resource optimization
- 📈 Logic-heavy simulation
- 🖥️ HTML, CSS, JavaScript

### ✅📋 DI-TO-DO-TASK
- 🧠 AI-inspired task manager
- 💎 Tailwind CSS UI
- ⚡ Fast & minimal frontend

### 🌱🤝 FirstLeaf
- 🐣 First PR helper for beginners
- 📄 NDJSON handling
- 🌍 Open-source ecosystem focus

---

## 📈🔥 Current Focus
- 🤖 Advanced AI/ML applications
- ⚙️ Scalable backend systems
- 🚀 Flagship projects for placements & startups

---

## 🤝📬 Connect With Me

📧 Email: **varunsoni01062004@gmail.com**  
💼 LinkedIn: [Divyansh Raj Soni](https://www.linkedin.com/in/divyanshrajsoni)  
🐦 Twitter/X: [@DivyanshRSoni](https://twitter.com/DivyanshRSoni)

---

⭐ _Building. Learning. Improving. Every day._ 🚀